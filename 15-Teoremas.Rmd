...
$f(x,z) = f_x(x)f_z(z)$, isto é, a função densidade conjunta é o produto das marginais.
...
Da primeira para a segunda igualdade, usamos a definição de esperança condicional. Da segunda para a terceira, usamos a independência de $X$ e $Z$ ($P_{X|Z}(x) = P_X(x)$).
...
Proposição 15.1.2 (independência na média para independência linear): Se $X$ e $Z$ são independentes na média, então são linearmente independentes. O reverso não é verdade.
...
&= \mathbb{E}[XZ]-\mu_X \cdot 0 \quad \text{(pois $\mu_Z = \mathbb{E}[Z] = 0$, já que $Z$ é simétrica em torno de zero)}\\
&= \mathbb{E}[XZ]\\
&= \mathbb{E}[Z^3] = 0
\end{aligned}
...
Definição 15.2 (viés). Seja $\theta_0$ o verdadeiro valor de um parâmetro de uma variável aleatória e seja $\hat{\theta}$ um estimador de $\theta_0$. O viés de um estimador é o desvio entre a esperança do estimador e o verdadeiro valor do parâmetro. 

$\text{Viés}(\hat{\theta}) = \mathbb{E}[\hat{\theta}] - \theta_0$.
...
Definição 15.3. (variância do estimador). Seja $\theta_0$ o verdadeiro valor de um parâmetro de uma variável aleatória e seja $\hat{\theta}$ um estimador de $\theta_0$. A variância de um estimador é o valor esperado do quadrado dos desvios amostrais.

$\mathrm{Var}[\hat{\theta}] = \mathbb{E}[\left(\hat{\theta} - \mathbb{E}[\hat{\theta}]  \right)^2]$

Proposição 15.3. Seja $Z_1, Z_2, ...$ uma sequência de variáveis aleatórias i.i.d tal que, para todo $i$, $\mathbb{E}[Z_i] = \mu$ e $\mathrm{Var}[Z_i] = \sigma^2$. Seja $X_n$ a média amostral das primeiras $n$ variáveis. A variância amostral é igual à variância de $Z$ dividida por $n$.

$\mathrm{Var}[X_n] = \frac{\mathrm{Var}[Z]}{n} = \frac{\sigma^2}{n}$
...
Mostre que se $X = Z^2$, então $X$ e $Z$ são não correlacionados, mas não independentes entre si.

2. Mostre que $X$ e $Z$ são independentes na média, mas não independentes entre si no seguinte caso: Seja $Y$ uma variável aleatória uniforme $Y \sim U(-1,1)$. Defina $X = Y+ \epsilon_1$ e $Z = Y+ \epsilon_2$, em que $\epsilon_1$ e $\epsilon_2$ são ruídos aleatórios com média zero, independentes entre si, $X$, $Z$ e $Y$.

Dica: em algum momento, use a definição (15.1.2) de que se $Cov(X,Z) \neq 0$, então $X$ e $Z$ não são independentes.
...
Como a esperança condicional é uma função de $X$ apenas, a esperança é calculada somando apenas com respeito a $X$.
...
A esperança interna condiciona em $X_1$ e $X_2$, e a externa apenas em $X_1$. E o resultado é o condicionamento apenas em $X_1$. Uma forma de memorizar o teorema é dizer que "o menor conjunto de informações ganha".

## Teorema do Condicionamento

Proposição: Se $\mathbb{E}[Y] < \infty$, então
