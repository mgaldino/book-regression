...
dados_binned <- dados_turno1 %>%
  filter(NM_MUNICIPIO == "LIMEIRA") %>%
  mutate(bin_1t = cut(perc_validos_1t, breaks = seq(0, 1, by = 0.05)))
...
# Criar o gráfico
grafico_cef <- ggplot() +
  geom_point(data = cef_data, aes(x = as.numeric(as.character(bin_1t)), y = perc_validos_2t_mean), 
             color = "red", size = 3) +
  geom_line(data = cef_data, aes(x = as.numeric(as.character(bin_1t)), y = perc_validos_2t_mean), 
            color = "red") +
