...
Uma das primeiras distinções que temos de fazer é sobre previsão e explicação (causal). Quando queremos prever, estamos interessados em saber quais os prováveis valores de variáveis no futuro, a partir de informações sobre a própria variável e outras no passado. Nesse sentido, é preciso algum tipo de suposição de que o futuro se assemelha ao passado de algum modo. Esse tipo de suposição usualmente toma a forma de um modelo probabilístico, mas não apenas.

Quando estamos interessados em explicações causais, temos dois tipos de perguntas de pesquisa possíveis. Uma sobre a chamada causa dos efeitos e outra sobre o efeito das causas (Gelman & Imbens, 2013). A causa dos efeitos são perguntas do tipo: o que causou a II Grande Guerra? Ou qual a causa da eleição de Trump ou Bolsonaro? O que explica a desigualdade de renda no Brasil? São todas perguntas em que queremos explicar um efeito, isto é, identificar as causas de um fenômeno (efeito). Já o efeito das causas são perguntas do tipo: qual o efeito da vacina de covid-19 sobre a mortalidade por Covid-19? Qual o efeito de checagem de notícias sobre a crença de pessoas em desinformação? Qual o efeito da magnitude eleitoral sobre fragmentação partidária? E assim por diante. Aqui, estamos interessados em entender o efeito causal de uma variável sobre outra, sem pretender esgotar todas as explicações de causa possíveis.
...
Nós iremos considerar que a regressão linear é uma forma de aproximar a chamada "Conditional Expectation Function" (CEF, na sigla em inglês). O objetivo é entender “as far as possible with the available data how the conditional distribution of some response $y$ varies across subpopulations determined by the possible values of the predictor or predictors” (Cook and Weisberg, apud Berk, p. 4).

## CEF

Para introduzir a CEF, iremos trabalhar com um banco de dados de eleição. O código abaixo mostra como importar e processar os dados.
...
# o arquivo dados_finais.rds está no repositório em:
...
O que constitui uma boa previsão? Tradicionalmente, empregamos a noção de Erro Quadrático Médio (EQM) para quantificar boas previsões. Quanto menor o EQM, melhor uma previsão. Se o objetivo é, portanto, fazer previsões que minimizem o EQM, iremos apresentar e mostrar que a Função de Esperança Condicional (*CEF*, na sigla em inglês) é o melhor preditor global possível. Vamos dizer, em outras palavras, porque esse resultado é verdadeiramente incrível. A CEF é o melhor preditor possível dentre todos que existam ou possam vir a existir, entendendo melhor como ter o menor EQM. Por isso que a CEF é o ponto de partida de qualquer preditor que exista, seja uma regressão simples ou algoritmos de aprendizado de máquina como "random forest" ou mesmo algoritmos de deep learning de redes neurais por trás dos recentes avanços na inteligência artificial.

> Mesmo os algoritmos mais avançados de inteligência artificial, como os **Large Language Models**, que estão na base de ferramentas como ChatGPT, *não podem* ter desempenho melhor que a função de esperança condicional, CEF, ao fazer uma previsão.

Naturalmente, se esse é o caso, a próxima pergunta que todos nós iremos fazer é: por que não aprender apenas a usar a CEF, que é o melhor preditor possível, e ser feliz para sempre? Porque a natureza não nos diz qual é a CEF. Nós nunca sabemos qual a verdadeira função de esperança condicional. Então tentamos aproximar o mais bem possível a CEF, a partir de simplificações da realidade. Em particular, nosso curso pode ser pensado em torno das seguintes perguntas: como aproximar a CEF por meio de regressão linear (combinação lineares de preditores)? Quais as propriedades dessa aproximação? Em que condições ela é uma boa aproximação e em que sentido (quantitativo e preciso) podemos falar de boa aproximação? Mais para o final do curso faremos a conexão entre a CEF, modelos preditivos e modelos causais.
...
E o que é a Esperança Condicional?
Qual o valor esperado do $Y$ para um dado valor de $X$. No gráfico, é dado pela linha vermelha.


## Algumas Propriedades da CEF

A CEF é simplesmente uma função de $X_i$ que resulta na esperança condicional, ou seja $\mathbb{E}[Y_i|X_i]$. É como se eu escrevesse $f(X_i) = \mathbb{E}[Y_i|X_i]$. 
...
Suponha que estamos interessados em achar a melhor função de $X$, $m(x)$ que aproxime os dados $Y$. Qual seria essa função? Se nós utilizamos o EQM para computar quão bem estamos aproximando os dados, então:
...
É muito comum a utilização de logaritmos em regressões, de modo que vale uma digressão sobre a correta interpretação de logaritmos. A primeira razão é que a diferença de logaritmos é aproximadamente igual à diferença entre porcentagens. Vamos ilustrar isso por meio de um exemplo, e então mostraremos matematicamente que isso sempre é o caso.

### Logaritmos como diferença de porcentagens

De acordo com dados da PNADc de 2017, quarto trimestre, o salário por hora dos homens é em média $14.72$ reais, e o das mulheres $12.57$ reais. A diferença percentual no salário é, portanto, $100*(14.72 - 12.57)/12.57$ ou $17,1%$. Se em vez de calcular a diferença percentual, usar a aproximação $log(14.72) - log(12.57) = 0.1578941$ ou 15,8%,  tenho um valor próximo do verdadeiro valor de 17,1%.
...
A média geométrica de um conjunto de observações $(x_1, x_2, \cdots, x_n)$ é dada por: $MG = \sqrt[n]{x_1 \cdot x_2 \cdots x_n}$.
...
Em palavras: A diferença na média de variáveis transformadas (pelo log) é aproximadamente a diferença percentual nas médias geométricas. Isso é relevante por causa do seguinte.
...
$$
Por essa razão, chamamos de: preditor linear de erro quadrático médio mínimo.

Por ora, basta saber que se minimizarmos a soma (ou a esperança) obteremos os valores de $\alpha$ e $\beta$. Quando falamos de uma amostra (em vez da população), iremos indicar esses valores por $\hat{\alpha}$ e $\hat{\beta}$.

Essa previsão, por mais que nós estejamos tentando minimizar o erro entre o valor do $y$ e previsto $\hat{y}$, em geral não será perfeito. Assim, para cada previsão $\hat{y}_i$ cometo um erro $\epsilon_i$ que é a diferença entre o observado $y$ e o previsto $\hat{y}$, ou seja:
...
Se eu substituir $\hat{y}_i$ por sua equação, temos: 
...
$$
Esse é chamado de modelo de regressão linear. Em resumo, o preditor linear é $\hat{y}_i = \alpha + \beta x_i$ e o modelo de regressão linear é $y_i = \alpha + \beta x_i + \epsilon_i$. No primeiro caso, tenho uma previsão que é diferente do observado. No segundo caso, acoplo a previsão ao erro e obtenho exatamente o observado.

Vale notar que minimizar a soma dos quadrados dos resíduos é o mesmo que minimizar a raiz quadrada dessa soma dividida por $n$, ou seja:
...
```{r regvoto_turnos_regressao_cap5,  message=FALSE, cache=TRUE, warning=F, fig.cap = "voto_por_municipio" }
...
Esse número implica que em média cometemos um erro ("resíduo médio") de .08 pontos percentuais. Ou seja, em média nosso modelo faz previsões diferentes do observado da ordem de .08 pontos percentuais.