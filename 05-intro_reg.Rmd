...
```{r #import-pnad, results='asis', message=FALSE, cache=TRUE}
...
```{r #chart-pnad, results='asis', message=FALSE, cache=TRUE}
library(ggplot2)

data %>%
  ggplot(aes(renda)) + geom_histogram()

data %>%
  ggplot(aes(renda, horas_trabalhadas)) + geom_point()
