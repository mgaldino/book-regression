...
knitr::opts_chunk$set(echo = TRUE, dpi=300,fig.width=7, scale=.8)
...
```{r, results='asis', message=FALSE, cache=TRUE}
...
pib_cid %>%
  ggplot(aes(y=pib_total, x=impostos)) + geom_point() +
  scale_y_continuous(labels = scales::dollar) + theme_barbie() +
  xlab("impostos municipais") + ggtitle("PIB municipal de 2013 x impostos municipais")
...
pib_cid %>%
  ggplot(aes(y=pib_per_capita)) + geom_histogram() +
  theme_light() + ggtitle("PIB per capita municipal")
