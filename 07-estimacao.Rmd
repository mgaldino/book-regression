...
&= \mathbb{Var}[\beta +  \frac{n^{-1}\sum_{i=1}^n({x_i - \bar{x}})e_i}{S_x^2}|X] \\
...
Uma definição alternativa é que o R-quadrado é a razão entre a covariância de $y$ e $\hat{y}$ e a variância de $Y$:
$$
...
Notem que: 
$$
\hat{\beta} = \frac{\sum({x_i - \bar{x}}) \cdot ({y_i - \bar{y}})}{\sum({x_i - \bar{x}})^2} = \frac{\sum({x_i - \bar{x}}) \cdot ({y_i - \bar{y}})}{n \cdot S^2_x} = \frac{n^{-1}\sum({x_i - \bar{x}}) \cdot ({y_i - \bar{y}})}{S^2_x}
$$
...
$$
\hat{\beta} = \frac{ n^{-1} \cdot (\alpha + \beta\bar{x})\sum({x_i - \bar{x}}) + n^{-1} \beta\sum(x_i - \bar{x})^2 + n^{-1} \sum({x_i - \bar{x}})e_i} {S^2_x}
$$
