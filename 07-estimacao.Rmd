...
Lembremos que a fórmula para o $\hat{\beta}$ é $\mathbb{Cov}[X,Y]/\mathbb{Var}[X]$. Substituindo nossa equação de regressão na fórmula, temos:

$$
\hat{\beta} = \frac{\sum({x_i - \bar{x}}) \cdot ({y_i - \bar{y}})}{\sigma^2_x} = \frac{\sum{(x_i - \bar{x}) \cdot y_i - (x_i - \bar{x})\bar{y}}}{\sigma^2_x} =  \frac{\sum({x - \bar{x}}) \cdot y_i - \sum({x -\bar{x}}) \cdot\bar{y}}{\sigma^2_x}
$$
