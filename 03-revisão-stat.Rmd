...
# ou podemos simplesmente usar mean(x)
mean(x)
```
...
1. Começamos expandindo o quadrado da esperança:
...
3. Pela linearidade da esperança, sabemos que $\mathbb{E}[A + B] = \mathbb{E}[A] + \mathbb{E}[B]$ (isso vale sempre, sem necessidade de independência). Então:
...
ex. 1.
Roleta. Em um cassino, um jogo comum é a roleta. Ela consiste normalmente de 37 números (0 a 36), e cada número tem uma cor (preto, vermelho ou verde). Ao girar a roleta, ela solta um número e uma cor. Portanto, podemos pensar que a roleta é uma distribuição conjunta de duas variáveis (números e cores).
