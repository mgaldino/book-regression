...
Regressão múltipla é mais facilmente compreendida com uso de matrizes. Portanto, vamos fazer uma rápida revisão da álgebra de vetores e matrizes.
...
$$
  ...
  \end{align}
$$
E posso fazer multiplicação de vetores (existem vários tipos, aqui me restringo ao produto interno ou produto ponto de vetores), desde que a gente multiplique um vetor linha por um vetor coluna, mas não o contrário.
...
A adição e multiplicação de matrizes é basicamente a generalização da álgebra com vetores
...
$$
Em um modelo sem interações, o efeito de cada variável $X_i$ é a contribuição separada para a resposta esperada (média). Portanto, $B_i$ mede a contribuição de como $\mathbb{E}[Y]$ muda à medida que $X_i$ (e apenas $X_i$) muda, para qualquer valor de $X_i$ (se a equação for linear nas variáveis) e para qualquer valor das demais variáveis (pressuposto de aditividade, sem interação, dos preditores).


Vamos retomar nosso modelo de previsão eleitoral e rodar no R, agora adicionando múltiplas variáveis.
...
Uma forma interessante de visualizar as previsões do modelo é que podemos escrever $\hat{Y} = X\hat{B}$. Substituindo a fórmula do $\hat{B}$, temos que:
$$
\hat{Y} = X (X'X)^{-1}X'Y = \\
(X (X'X)^{-1}X')Y = \\
HY
...
Softwares modernos, como R, irão "dropar" uma (ou mais) variável(eis) se isso ocorrer, automaticamente, para evitar que a matriz não seja invertível. Assim, a menos que a correlação seja perfeita, multicolinearidade não costuma ser um problema.
...
$$
Lembrem-se que a variância do $\hat{\beta}|X$ no modelo de regressão simples era dada por: $\frac{\sigma^2}{nS_x^2}$

Então, $\frac{\sigma^2}{n}$ é igual ao que tínhamos antes. À medida que $n$ cresce, esperamos que $X'X$ cresça, já que é uma soma sobre todos os dados $n$. Dividindo todas as entradas da matriz por $n$ compensa isso. Se a covariância amostral entre todos os preditores fossem iguais a zero (sem correlação), então quando calculássemos a inversa obteríamos apenas a variância amostral de $X$ $1/S_x^2$ na diagonal principal, e temos um termo que já conhecemos de regressão simples. Lá, como só tem um preditor, não tem como ter covariância com outro preditor.
...
Eu suponho que isso seja engraçado de alguma forma (ou talvez fosse uma forma mnemônica de memorizar a equação). Mas o fato é que o termo sanduíche pegou de tal forma que nosso erro padrão robusto envolve trocar a "carne".
...
A questão então é como calcular essas variância diferente para esse pontos. Há inúmeras maneiras de fazer isso. A default do Stata, chamada de "HC1", em que "HC" quer dizer "Heteroscedasticity-Consistent". A fórmula para o erro padrão robusto "HC1" é a seguinte:
$$
\frac{n}{n-k}\hat{e}^2
$$
...
$$
Aqui, $h_i$ são os valores chapéus da matriz chapéu, $H$, que vimos antes. Os valores chapéus variam entre $0$ e $1$ e quanto maior o número, mais influente a observação. Então, sabemos que nossos dois últimos números terão valores chapéu maiores, ou seja, vão inflar a variância para esses dois valores. No R, podemos calcular os valores chapéus com a função "hatvalues".
...
Agora, $\beta_1$ mede o efeito no $y$ médio de aumentar uma unidade $x_1$ quando $x_2 = 0$. Para ver isso, podemos substituir $x_2 = 0$ na equação acima, e temos:
...
Portanto, a interpretação do que é o efeito marginal ou preditivo Em um modelo com termo de interação é mais complexa do que em uma regressão sem termo de interação.
...
Lembrem-se que para um único preditor, $\mathbb{Var}[\beta|X] = \frac{\sigma^2}{nS^2_x}$
...
$$
E em geral a variância de cada coeficiente depende dos demais. E quanto maior a covariância entre duas variáveis, maior a variância (e portanto, maior o erro-padrão).
...
$$
Var(\beta_1 + \beta_3 x_2|X) = Var(\beta_1|X) + Var(\beta_3 x_2|X) + 2Cov(\beta_1, \beta_3) = Var(\beta_1|X) + x_2^2Var(\beta_3|X) + 2x_2Cov(\beta_1, \beta_3|X)
$$
...
Lembremos que $\sum{\hat{e}_i} = 0$ e $\sum{\hat{e}_i\hat{y}_i} = 0$
\begin{align}
SST = \sum{(y_i - \bar{y})^2} = \\
\sum{(\hat{e}_i + \hat{y}_i - \bar{y})^2} = \\
\sum{(\hat{e}_i + (\hat{y}_i - \bar{y}))^2} = \\
\sum{\hat{e}_i^2 + 2\hat{e}_i(\hat{y}_i - \bar{y}) + (\hat{y}_i - \bar{y})^2} = \\
\sum{\hat{e}_i^2} + 2\sum{\hat{e}_i(\hat{y}_i - \bar{y})} + \sum{(\hat{y}_i - \bar{y})^2} = \\
SSE + 2\sum{(\hat{e}_i\hat{y}_i - \hat{e}_i\bar{y})} + SSM = \\
SSE + 2\sum{(\hat{e}_i\hat{y}_i)} - 2\sum{(\hat{e}_i\bar{y})} + SSM = \\
SSE + 0 - 0 + SSM = \\
SST = SSE + SSM = \\
\end{align}
...
$$
\hat{<viés>} = \hat{\beta}_2 \cdot \hat{\gamma}
$$
