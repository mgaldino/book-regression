...
Diante do problema de viés de variável omitida, podemos realizar análises de sensibilidade. Essa é uma área que em que houve bastante desenvolvimentos recentes. Apresento aqui dois dos principais testes disponíveis. O primeiro é o desenvolvido pela economia Emily Oster (2019).

Quando falamos em identificação causal, é comum dividir as variáveis em observáveis (aquelas que podemos observar e mensurar e, portanto, incluir na regressão) e não-observáveis (aquelas que não conseguimos observar e, portanto, medir). O problema de variávem omitida é mais sério, obviamente, quando se trata de uma variável não-observável. Como sempre, vamos considerar que tenho um tratamento de interesse e as demais variáveis são apenas controle. O pressuposto dos testes é que consigo estimar de algum modo a relação entre tratamento e não-observáveis a partir da relação entre tratamento e observáveis.

Oster vai tratar do seguinte caso simples. Imagine que a verdadeire regressão em um caso qualquer é dada pela seguinte equação: $Y = \alpha + \beta T + \varphi X + \omega Z + e$, em que $Y$ é a variável resposta, $T$ o tratamento, $\alpha$ e $\beta_$) parâmetros usuais, e $X$ um vetor de controles (observáveis) e $Z$ um vetor de variáveis não observáveis (omitidas). Vamos supor que $X$ e $Z$ são ortogonais (isto é, covariância zero), e $e$ o termo de erro.

Suponha que a relação entre $T$ e $Z$ seja dada pelo DAG abaixo, em que $U$ é um termo de erro:
...
Se essa relação for linear, podemos escrevê-la como: $Z = \psi + \eta T + u$. Ao não incluirmos $Z$ na regressão, temos viés de variável omitida, pois nossa equação de regressão fica:
\begin{align}
Y = \alpha + \beta T +  \varphi X + \omega\psi + \omega\eta T + \omega u +  e \\
Y = (\alpha + \omega\psi) + \beta T + \omega\eta T + \varphi X  + \omega u +  e \\
Y = (\alpha + \omega\psi) + (\beta + \omega\eta) T + \varphi X  + \omega u +  e \\
\end{align}

$$
Y = (\alpha + \omega\psi) + (\beta + \omega\eta) T + \varphi X  + \omega u +  e 
$$
Ou seja, estiamos $\beta_{bias} = \beta + \omega\eta$, que é uma estimativa viesada do $\beta$.


E vamos assumir que $Var(W) >>> Var(C)$.

## Alternativas
Nós vimos que o viés de variável omitida (no caso de uma variável omitida), em uma amostra, pode ser explicitado por:
$$
...
Vamos mostrar que, do memso jeito que $\hat{T} = \hat{M} + \hat{E}$, $SST = SSM + SSE$
...
$$
R^2 = \frac{SSM}{SST} = Var(\hat{y})/Var(y)
$$
E pode ser escrito também como:
$$
R^2 = \frac{SSM}{SST} = \frac{SSM}{SSM + SSE} = \frac{\frac{SSM}{SSM}}{\frac{(SSM + SSE)}{SSM}} = \frac{1}{1 + \frac{SSE}{SSM}} = \frac{1}{1 + \frac{SST - SSM}{SSM}} = \frac{1}{1 + \frac{SST}{SSM} - 1} = \frac{1}{\frac{SST}{SSM}} = \frac{SSM}{SST} = 
$$
Seja a Soma dos Quadrados do Resíduos (SQR), $\sum{\hat{e}^2} = \sum{(y_i - \hat{y}_i)^2}$ 

E seja a Soma dos Quadados Totais (SQT) dada por: $\sum{(y_i - \bar{y})^2}$

O $R^2$ pode ser definido como a proporção da variância de $Y$ predita pelas variáveis independentes relativa à variância total de $Y$. 

A Soma dos Quadrados Prevista (SQP) pode ser definida como:  $\sum{(\hat{y}_i - \bar{y})^2}$, ou seja, a soma da diferença (a quadrado) entre a previsão e a média (modelo só com intercepto).

https://rpubs.com/beane/n3_1b

$$
R^2 = SQP/SQT = \frac{\sum{(\hat{y}_i - \bar{y})^2}}{\sum{(y_i - \bar{y})^2}}
$$

Veja que $SQT = SQP + SQR$, pois
$$
\sum{(y_i - \bar{y})^2} = //
\sum{(y_i - \bar{y} + \hat{y}_i - \hat{y}_i)^2} = //
\sum{(\hat{y}_i - \bar{y} + y_i - \hat{y}_i)^2} = //
\sum{[(\hat{y}_i - \bar{y}) + (y_i - \hat{y}_i)]^2} = //
\sum{[(\hat{y}_i - \bar{y}) + (\hat{e}_i)]^2} = //
\sum{(\hat{y}_i - \bar{y})^2} + 2 \sum{\hat{e}_i(\hat{y}_i - \bar{y})} + \sum{\hat{e}_i^2} = //
\sum{(\hat{y}_i - \bar{y})^2} + \sum{\hat{e}_i^2} + 2 \sum{\hat{e}_i(X\hat{B} - \bar{y})}  = //
$$
## Checagem do modelo