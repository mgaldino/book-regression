...
```{r package loads 10, message=FALSE}
library(ggplot2)
library(knitr)
library(tidyverse)
library(electionsBR)
...
```{r residuos-preditor, message=FALSE}
...
Como exercício, verifique que isso é verdade matematicamente (pós) e por simulação (pós e graduação). Abaixo segue uma explicação:
...
$$
Portanto, se estamos assumindo homecadisticidade, isto é, que $\mathbb{Var}[\hat{e}|X] = \sigma^2$, podemos checar esta suposição olhando para a esperança condicional do quadrado dos resíduos. E odemos fazer isso plotando o gráfico do quadrado dos resíduos contra o preditor.
...
Podemos fazer um plote dos resíduos contra seção/zona/município, zona/município ou município. Vamos tentar o primeiro, que possui mais granularidade.Vejamos como fica.
...
df <- data.frame(residuos = residuals(reg1), secao = as.numeric(presid_al18_valido$id_secao))

df %>%
  ggplot(aes(x=secao, y = residuos)) + geom_point() + geom_smooth(method="lm", se=F)
```

Uma técnica simples, que é uma versão simplificada de algo mais formal chamado "permutation test", é plotar os resíduos permutados aleatoriamente contra a data ou espaço (no nosso caso, espaço). Vamos usar o pacote [nullabor](https://dicook.github.io/nullabor/) library(nullabor)

```{r residuos-shufle-coordenadas, message=FALSE}
library(nullabor)
...
df <- data.frame(residuos_s = sample(residuals(reg1)), secao = as.numeric(presid_al18_valido$id_secao))

df %>%
  ggplot(aes(x=secao, y = residuos_s)) + geom_point() + geom_smooth(method="lm", se=F)
```

Rodando o código algumas vezes, podemos verificar que o "jeitão" dele é muito parecido com o nosso gráfico na ordem original, o que sugere que está tudo bem com nossos resíduos com relação à correlação espacial deles.
